...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev, int high_speed) {
	...
	pcd_fs_handle.Init.low_power_enable = DISABLE;
	pcd_fs_handle.Init.lpm_enable = DISABLE;
	pcd_fs_handle.Init.battery_charging_enable = DISABLE;
	if (HAL_PCD_Init(&pcd_fs_handle) != HAL_OK)
	{
		//Error_Handler( );
	}
	...
	addr += 32;
	HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , 0x05 , PCD_SNG_BUF, addr);
	addr += 32;
	HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , 0x05 , PCD_SNG_BUF, addr);
	addr += 32;
	HAL_PCDEx_PMAConfig((PCD_HandleTypeDef*)pdev->pData , 0x84 , PCD_SNG_BUF, addr);
	#endif
